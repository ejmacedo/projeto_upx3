...
# Função para carregar e tratar os dados do Excel
@st.cache_data(show_spinner=False)
def carregar_dados(caminho_excel):
    df = pd.read_excel(caminho_excel)
    # Padroniza o texto em letras maiúsculas para evitar inconsistências na filtragem
...
# Define as opções únicas para os filtros
...
# Exibe os dados filtrados em forma de tabela
with st.container():
...
if not df_filtrado.empty:
    ...
    # Mostra os indicadores estatísticos principais
    ...
    # Conversões explicativas para facilitar o entendimento
    casas_equivalentes = consumo_total / 0.00000864
    carros_equivalentes = emissoes_total / 2

    # Mostra o impacto ambiental em termos leigos
    ...
    # Simulação da substituição parcial por energia solar
    ...
    energia_kwh = consumo_solar * 277778  # Conversão de TJ para kWh
    ...
    st.info("Esses valores simulam o impacto positivo da energia solar: menor dependência da rede elétrica, economia nas contas de luz e contribuição direta com o meio ambiente.")

    # Gráficos de linha para evolução anual com base nos filtros
    ...
    # Gráfico de barras com as 10 categorias que mais consomem energia
